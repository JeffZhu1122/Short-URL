...
<div class="panel panel-primary">
	<div class="panel-heading" style="background: #15A638;">
		<h3 class="panel-title" align="center">GZ防红工具</h3>
	</div>
	<div class="panel-body">
		<p><iframe src="../readme.php" style="width:100%;height:465px;"></iframe></p>
		<?php if($installed){ ?>
...
if(defined("SAE_ACCESSKEY") || (ini_get('acl.app_id') && !preg_match('/php-tae-temp/',PHP_BINDIR)) || $_GET['jump']==1){
	...
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			if(DB::query("select * from frame_host where 1")==FALSE)
				echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
			else
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过5A防红</div>
...
<div class="panel panel-primary">
	<div class="panel-heading">
		<h3 class="panel-title" align="center">添加域名</h3>
	</div>
<div class="progress progress-striped">
  <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 85%">
	...
	<div class="panel-body">
		...
		<label for="name">接口域名(不带http://和/):</label>
...
if($domain==NULL ){
	...
	$cn = DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port']);
	$sql = "insert into `frame_host` (`domain`,`date`,`type`) values ('".$domain."','2017-10-27 13:22:42','1')";
	$ad=DB::query($sql);
	if($ad){
		@file_put_contents("install.lock",'安装锁');
		echo '<div class="alert alert-info"><font color="green">安装完成！管理账号和密码是:admin/admin</font><br><font color="green">config.php文件中修改。</font><br><br><a href="../">&gt;&gt;网站首页</a><hr><br>config.php中可以修改账号密码<br><font color="#FF0033">如果你的空间不支持本地文件读写，请自行在install/ 目录建立 install.lock 文件！</font></div>';
	}else{
